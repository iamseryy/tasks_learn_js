## Task8. Основы событий в JavaScript
...
<body>
    ...
    <script>
        ...
        document.addEventListener('click', e => console.log(`Класс "super_element" ` +
            `${e.target.classList.contains('super_element') ? "присутствует" : "отсутствует"} ` +
            `в элементе "${e.target.tagName.toLowerCase()}".`)
        )

        // 4. Сделайте, чтобы при наведении на textarea в консоли появлялось сообщение: "Вы навели на textarea."
        document.querySelector('textarea').addEventListener('mouseover', e => console.log('Вы навели на textarea.'))
        ...
        // Ответ: Разница в длине цепочек фаз перехвата и всплытия события.
        // В третьем задании элементов при прохождении события больше, поэтому и текст из 3 задания появляется позже.

        // 7. С помощью JS необходимо изменить цвет заднего фона каждого второго тега li.
        document.querySelectorAll('li:nth-child(2n)').forEach(element => element.style.backgroundColor = 'red')
    </script>
</body>
</html>
```